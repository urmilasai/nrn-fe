    ...
    <div className="w-full bg-white dark:bg-gray-800  py-6 rounded-lg">
      <div className="relative mb-6">
        <div className="flex items-center justify-between">
          ...
          { news?.length > 8 && <div className="ms-2">
            <Link href="/news" className="text-sm text-gray-500 dark:text-gray-400 hover:text-[#1a3869] transition-all duration-300 ease-in-out cursor-pointer">
              View All
            </Link>
          </div>}
        ...
        {newsLoading ?
          ...
          : news?.map((story: NewsItem, index: number) => (
            <div key={index} className="bg-white dark:bg-gray-700 rounded-lg shadow-md overflow-hidden cursor-pointer transition-transform duration-300 hover:scale-105" onClick={() => router.push(`/news/${story?._id}`)}>
              ...
              <div className="p-4">
                <div className="text-xs bg-primary text-white px-2 py-1 rounded-md  inline-flex">
                  <span>

                    {story.breaking === true && `Breaking`}
                    {story.trending === true && `Trending`}
                  </span>
                  <span className='text-capitalize ps-1'>
                    {story.category}
